...
import {
  ...
  Volume2,
  X,
} from "lucide-react"; // Ícones adicionais
...
export function MusicianProfileForm({ initialData }: MusicianProfileFormProps) {
  ...
  const handleArrayChange = (
    item: string,
    isChecked: boolean,
    field: keyof MusicianProfileData,
  ) => {
    setFormData((prev) => {
    ...
    reader.onloadend = () => {
      if (type === "avatar") {
        ...
        if (type === "image") {
          setFormData((prev) => ({
            ...prev,
            portfolioImages: [...prev.portfolioImages, newAsset],
          }));
          toast.success("Imagem adicionada ao portfólio!");
        } else if (type === "audio") {
          setFormData((prev) => ({
            ...prev,
            portfolioAudios: [...prev.portfolioAudios, newAsset],
          }));
          toast.success("Áudio adicionado ao portfólio!");
        } else if (type === "video") {
          setFormData((prev) => ({
            ...prev,
            portfolioVideos: [...prev.portfolioVideos, newAsset],
          }));
  ...
  const removeMedia = (
    idToRemove: string,
    field: "portfolioImages" | "portfolioAudios" | "portfolioVideos",
  ) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const validateForm = () => {
    ...
    if (!formData.specialty)
      newErrors.specialty = "Especialidade principal é obrigatória.";
    if (formData.genres.length === 0)
      newErrors.genres = "Selecione pelo menos um gênero musical.";
    if (formData.instruments.length === 0)
      newErrors.instruments = "Selecione pelo menos um instrumento.";
    ...
    if (!formData.contactPhone.trim()) {
      ...
      !/^\(?\d{2}\)?\s?\d{4,5}\-?\d{4}$/.test(
        formData.contactPhone.replace(/\D/g, ""),
      )
    ) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid h-auto w-full grid-cols-2 flex-wrap gap-2 md:grid-cols-4 md:gap-0 lg:grid-cols-5">
          ...
          <TabsTrigger value="social" className="hidden lg:block">
            Redes Sociais
          </TabsTrigger>{" "}
          {/* Opcional: aba separada para redes sociais */}
        </TabsList>

        {/* Tab 1: Informações Pessoais */}
        <TabsContent value="personal" className="mt-6 space-y-6">
          <h2 className="text-foreground text-xl font-bold">Sua Essência</h2>

          {/* Avatar */}
          <div className="flex flex-col items-center gap-4">
            <Avatar className="border-primary h-24 w-24 border-2 shadow-md">
              <AvatarImage
                src={formData.avatarUrl || "/placeholder-avatar.png"}
                alt="Avatar do Músico"
              />
              <AvatarFallback className="bg-muted-foreground text-muted text-xl font-semibold">
          ...
          <div>
            ...
            />
            {errors.bio && (
              <p className="text-destructive mt-1 text-sm">{errors.bio}</p>
            )}
          </div>

          <div>
            <Label htmlFor="location" className="flex items-center gap-1">
              <MapPin className="text-muted-foreground h-4 w-4" /> Localização
              (Cidade/Estado)
            </Label>
            <Input
              id="location"
            ...
            />
            {errors.location && (
              <p className="text-destructive mt-1 text-sm">{errors.location}</p>
            )}
          </div>
        </TabsContent>

        {/* Tab 2: Detalhes Musicais */}
        <TabsContent value="musical" className="mt-6 space-y-6">
          ...
          <div>
            ...
            <div className="mt-2 grid max-h-60 grid-cols-2 gap-3 overflow-y-auto pr-2 sm:grid-cols-3 md:grid-cols-4">
                ...
                <div key={instrument} className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      handleArrayChange(instrument, !!checked, "instruments")
                    }
                    disabled={isLoading}
                  />
          ...
          <div>
              ...
              {musicalGenres.map((genre) => (
                <div key={genre} className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      handleArrayChange(genre, !!checked, "genres")
                    }
                    disabled={isLoading}
                  />
            ...
            {errors.genres && (
              <p className="text-destructive mt-1 text-sm">{errors.genres}</p>
            )}
          </div>
          ...
          <div>
            ...
            <RadioGroup
              ...
              onValueChange={(value: typeof formData.experienceLevel) =>
                handleSelectChange(value, "experienceLevel")
              }
              ...
              {skillsList.map((skill) => (
                <div key={skill} className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      handleArrayChange(skill, !!checked, "skills")
                    }
                    disabled={isLoading}
                  />
        ...
        <TabsContent value="media" className="mt-6 space-y-6">
          <h2 className="text-foreground text-xl font-bold">
            Seu Portfólio de Mídia
          </h2>
          <p className="text-muted-foreground text-sm">
            Adicione fotos, áudios e vídeos para mostrar seu trabalho.
          </p>
          ...
          <div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
              {formData.portfolioImages.map((img) => (
                <div
                  key={img.id}
                  className="group border-border relative overflow-hidden rounded-md border shadow-sm"
                >
                  <Image
                    src={img.url}
                    alt={`Portfólio Imagem ${img.id}`}
                    width={128}
                    height={128}
          ...
          <div>
            <Label className="mb-2 flex items-center gap-2">
              <Volume2 className="h-4 w-4" /> Áudios (Links do
            ...
            <Button
              ...
              onClick={() =>
                toast.info(
                  "Funcionalidade de adicionar link de áudio em desenvolvimento.",
                )
              }
            >
            ...
            <div className="mt-4 space-y-2">
              {formData.portfolioAudios.map((audio) => (
                <div
                  key={audio.id}
                  className="bg-muted text-muted-foreground flex items-center justify-between rounded-md p-2"
                >
          ...
          <div>
            <Label className="mb-2 flex items-center gap-2">
              <Video className="h-4 w-4" /> Vídeos (Links do YouTube/Vimeo)
            </Label>
            <Input
              id="newVideoLink"
              type="url"
            ...
            <Button
              ...
              onClick={() =>
                toast.info(
                  "Funcionalidade de adicionar link de vídeo em desenvolvimento.",
                )
              }
            >
        ...
        <TabsContent value="contact" className="mt-6 space-y-6">
          ...
          <div>
            <Label htmlFor="contactPhone" className="flex items-center gap-1">
              <Phone className="text-muted-foreground h-4 w-4" /> Telefone de
              Contato
            </Label>
            <Input
              id="contactPhone"
          ...
          <div>
            <Label htmlFor="contactEmail">
              E-mail de Contato (Opcional, se diferente do cadastro)
            </Label>
            <Input
              id="contactEmail"
              type="email"
          ...
          <div>
            <Label htmlFor="priceRange">
              Faixa de Preço (por apresentação/evento, opcional)
            </Label>
            <Input
              id="priceRange"
              type="text"
          ...
          <div>
            ...
            <div className="mt-2 grid max-h-60 grid-cols-2 gap-3 overflow-y-auto pr-2 sm:grid-cols-3">
              {availabilityOptions.map((option) => (
                <div key={option} className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      handleArrayChange(option, !!checked, "availability")
                    }
                    disabled={isLoading}
                  />
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              ...
              onCheckedChange={(checked) =>
                handleSelectChange(String(checked), "acceptsTravel")
              }
              disabled={isLoading}
            />
            <Label htmlFor="acceptsTravel">
              Aceito viajar para apresentações
            </Label>
          </div>
        </TabsContent>
        ...
        <TabsContent value="social" className="mt-6 space-y-6">
          ...
          <p className="text-muted-foreground text-sm">
            Adicione links para seus perfis em plataformas como Instagram,
            ...
            <div>
              ...
              <div className="mt-1 flex gap-2">
                ...
                <Button
                  ...
                  onClick={() =>
                    toast.info(
                      "Funcionalidade de adicionar link social em desenvolvimento.",
                    )
                  }
                >
                  <Plus />
